    ...
    for i, c2w in enumerate(render_poses):
        ...
        if obj is None:
            ...
            for r, render_set_i in enumerate(render_set):
                ...
                # for k in rm_ls[i]:
                #     obj_i[k] = np.ones([9]) * -1
                # for camera_param in cam_aug_params:
                #     novel_c2w = make_camera_like_input_camera(c2w, **camera_param)
                #     rgb, disp, acc, _ = render(
                #     H, W, focal, chunk=chunk, c2w=novel_c2w[:3, :4], obj=obj_i, **render_kwargs)
                #     rgbs.append(rgb.numpy())
                #     disps.append(disp.numpy())

                """
                c2w, add_xrot_val=0, add_zrot_val=0, add_tz=0
                """
                for cam_aug in cam_aug_params:
                    new_c2w = make_camera_like_input_camera(c2w,**cam_aug)
                    rgb, disp, acc, _ = render(
                            H, W, focal, chunk=chunk, c2w=new_c2w[:3, :4], obj=None, time_stamp=time_st, **render_kwargs)
                    rgbs.append(rgb.numpy())
                    disps.append(disp.numpy())  
...
def create_nerf(args):
    ...
    if args.use_object_properties and not args.bckg_only:
        ...
        # Version b: One Network for all similar objects of the same class
            ...
            for obj_class in args.scene_classes:
                ...
                grad_vars += model_obj.trainable_variables
                models[model_name] = model_obj
                models_dynamic_dict[model_name] = model_obj # model_class_0000_~~~ 
    ...
    if args.ft_path is not None and args.ft_path != 'None':
        ...
        for f in sorted(os.listdir(args.model_library)):
            if 'model_' in f and 'fine' not in f and 'optimizer' not in f and 'class' not in f and 'obj' not in f: # 지금 model 전체랑 model_class 해서 모두 ckpts 로 들어가고 있음
            ...
            if 'class' in f:
                make_num = f.split('_')
                make_num_obj = make_num[-2]
                make_num_model = make_num[-1][:-4]
            if 'class' in f and float(make_num_obj) in args.scene_classes: # latent vector obj 들이 obj ckpts 로 들어가는 중
                obj_ckpts[make_num[0] + '_' + make_num[1]+ '_' + make_num[-2]] = (os.path.join(args.model_library, f))
    ...
    if len(ckpts) > 0 and not args.no_reload and (not args.obj_only or args.model_library):
        ft_weights = ckpts[-1]
        # ft_weights = '/home/nas4_user/minjungkim/Others/kang/KITTI_NSG/example_weights/kitti_tracking_0006/model_120000.npy'
...
def train():
    ...
    starts = args.first_frame
    ends = args.last_frame
