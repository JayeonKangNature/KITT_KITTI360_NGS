...
from Test_render.cameras import *
...
import cv2
from Test_render.cameras import *
from Test_render.general_utils import *
from Test_render.graphics_utils import *
from Test_render.Test_rendering import *
import torchvision
...
def render_path(render_poses, hwf, chunk, render_kwargs, obj=None, obj_meta=None, gt_imgs=None, savedir=None,
                render_factor=0, render_manipulation=None, rm_obj=None, time_stamp=None, novel_view = None):
    ...
    '''
    make_camera_like_input_camera(c2w, add_xrot_val=0, add_zrot_val=0, add_tz=0) --> c2w
    '''
    ...
    for i, c2w in enumerate(render_poses):
        ...
        if obj is None:
            ...
            #     #     a, b, c = cam_aug_param
            #     #     c2w_novel = make_camera_like_input_camera(c2w, a, b, c)
            #     #     rgb, disp, acc, _ = render(
            #     #     H, W, focal, chunk=chunk, c2w=c2w_novel[:3, :4], obj=None, time_stamp=time_st, **render_kwargs)
            #     #     rgbs.append(rgb.numpy())
            #     #     disps.append(disp.numpy())
            #     cam_aug_param = cam_aug_params[0]
            #     a, b, c = cam_aug_param
            #     c2w = make_camera_like_input_camera(c2w, a, b, c)
            ...
            for r, render_set_i in enumerate(render_set):
                ...
                # for cam_aug_params in cam_aug_params:
                #     c2w_novel = make_camera_like_input_camera(c2w, **cam_aug_param)
                #     rgb, disp, acc, _ = render(
                #         H, W, focal, chunk=chunk, c2w=c2w_novel[:3, :4], obj=obj_i, **render_kwargs)
                #     rgbs.append(rgb.numpy())
                #     disps.append(disp.numpy())
                ...
                # if novel_view is 'go_all':
                #     for cam_aug in cam_aug_params:
                #         a, b, c = cam_aug
                #         c2w = make_camera_like_input_camera(c2w, a, b, c)
                #         rgb, disp, acc, _ = render(
                #             H, W, focal, chunk=chunk, c2w = c2w[:3, :4], obj=obj_i, **render_kwargs)
                ...
                if novel_view != None:
                    novel_view = int(novel_view)
                    a,b,c = cam_aug_params[novel_view]
                    c2w = make_camera_like_input_camera(c2w, a, b, c)
                    rgb, disp, acc, _ = render(
                    H, W, focal, chunk=chunk, c2w = c2w[:3, :4], obj=obj_i, **render_kwargs)
                    rgbs.append(rgb.numpy())
    ...
    if args.ft_path is not None and args.ft_path != 'None':
        ...
        for f in sorted(os.listdir(args.model_library)):
            ...
            if 'class' in f:
                spt_f = f.split('_')
                obj_num = spt_f[-2]
            #if 'class' in f and float(f[10:][:-11]]) in args.scene_objects:
            if 'class' in f and float(obj_num) in args.scene_classes:
...
def config_parser():
    ...
    parser.add_argument("--novel_view", default=None, help='input data directory') # adding
