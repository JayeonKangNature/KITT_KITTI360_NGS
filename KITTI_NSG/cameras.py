...
import numpy as np
# from graphics_utils import getWorld2View2, getProjectionMatrixwithPrincipalPointOffset, focal2fov
from PIL import Image
# from general_utils import Normal2Torch
...
def make_camera_like_input_camera(c2w, add_xrot_val=0, add_zrot_val=0, add_tz=0):
    '''
    ...
    opengl2kitti = np.array([[1, 0, 0],
                             [0, -1, 0],
                             [0, 0, -1]])

    kitti2vkitti = np.array([[1., 0., 0.],
                             [0., 0., -1.],
                             [0., 1., 0.]])
    ...
    R_z = np.array([[np.cos(phi), 0, np.sin(phi)], [0, 1, 0], [-np.sin(phi), 0, np.cos(phi)]]) 
   
    # R_z = np.matmul(R_z, opengl2kitti)
    # R_z = np.matmul(kitti2vkitti, R_z)
